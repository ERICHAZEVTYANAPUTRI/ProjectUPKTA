...
const CustomDropdownMenu = ({ label, icon, items, open, isOpen, setIsOpen }) => {
  const location = useLocation();

  // Cek apakah ada child yang aktif
  const isAnyItemActive = items.some(({ to, activePaths = [] }) => [to, ...activePaths].some((path) => location.pathname.startsWith(path)));

  // ✅ Aktifkan parent hanya jika dropdown ditutup
  ...
  return (
    <>
      ...
      >
        ...
        {open && (isOpen ? <MdKeyboardArrowUp /> : <MdKeyboardArrowDown />)}
      </ListItemButton>
      ...
      <Collapse in={isOpen} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          {items.map(({ to, label, activePaths = [] }) => {
            const isActive = [to, ...activePaths].some((path) => location.pathname.startsWith(path));
