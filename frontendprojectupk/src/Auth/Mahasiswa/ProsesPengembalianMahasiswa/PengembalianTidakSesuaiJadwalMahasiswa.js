...
const HalamanMahasiswaPengembalianTidakSesuaiJadwal = () => {
  ...
  const diterimaPengajuan = pengajuan.filter(
    (item) => item.status === "diterima" && String(item.mahasiswapenanggungjawab) === String(user?.id) && item.statuspeminjaman === "prosespengembalian" && item.statusuploadvidio && item.statusuploadvidio.trim() !== ""
  );
  ...
  const displayedItems = filteredPengajuan.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
  ...
  const handleKirimDriveLink = async () => {
    ...
    try {
      ...
      // Remove the updated pengajuan from list to refresh UI
                    ...
                    endAdornment: (
                      <InputAdornment position="end">
                        <Button
                          ...
                          onClick={() => setCurrentPage(0)} // reset page saat search
                        >
                          <BsSearch />
                        </Button>
                      </InputAdornment>
              ...
              <TableContainer sx={{ border: "1px solid #e5e6ed", borderRadius: 1 }}>
                <Table>
                  <TableHead sx={{ backgroundColor: "#3449e4" }}>
                    <TableRow>
                      {["No", "Nama Mahasiswa", "Kelas", "Mata Kuliah", "Dosen", "Nama Ruangan", "Jam", "Info"].map((header, index, arr) => (
                        <TableCell
                          key={header}
                          sx={{
                            color: "white",
                            ...
                            paddingTop: 1,
                            paddingBottom: 1,
                            borderRight: index !== arr.length - 1 ? "2px solid white" : "none",
                            textAlign: "center",
                          }}
                        >
                          {header}
                      ...
                      <TableRow key={item.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#f5f5f5" : "#fff" }}>
                        ...
                        <TableCell align="center" sx={{ borderRight: "2px solid #e5e6ed" }}>
                          ...
                          >
                            ...
                            >
                              <div style={{ marginBottom: "3px" }}>Proses</div>
            ...
            <Modal open={showPopup} onClose={() => setShowPopup(false)}>
              <Box sx={styleModal}>
                ...
                <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 3 }}>
                  ...
                  <Button variant="contained" onClick={handleKirimDriveLink}>
                    Kirim
                  </Button>
                </Box>
              </Box>
