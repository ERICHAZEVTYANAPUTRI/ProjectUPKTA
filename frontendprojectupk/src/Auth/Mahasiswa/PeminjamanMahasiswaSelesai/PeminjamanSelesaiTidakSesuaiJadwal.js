...
const HalamanMahasiswaPengembalianSelesaiTidakSesuaiJadwal = () => {
  ...
  const matakuliahOptions = [
    ...Array.from(new Set(pengajuan.map((item) => item.matakuliah?.namamatakuliah)))
      .filter((nama) => nama)
  ...
  const pageOptions = [
    { value: "/peminjamanselesaisesuaijadwalbaianmahasiswa", label: "Pengembalian Selesai Sesuai jadwal" },
    { value: "/pengembalianselesaimahasiswa", label: "Pengembalian Selesai Tidak SesuaiJadwal" },
  ];
  ...
  useEffect(() => {
    const fetchUserAndPengajuan = async () => {
      try {
        ...
        const filtered = semuaPengajuan.filter((item) => item.status === "selesai" && item.statuspeminjaman === "selesai" && item.statusuploadvidio?.trim() !== "" && String(item.mahasiswa_id) === String(userData.id));
  ...
  const diterimaPengajuan = pengajuan.filter((item) => item.status === "selesai" && item.statuspeminjaman === "selesai" && item.statusuploadvidio && item.statusuploadvidio.trim() !== "" && String(item.mahasiswa_id) === String(user?.id));
  ...
  const filteredPengajuan = diterimaPengajuan.filter((item) => {
    ...
    const matchesSearch = nama.includes(searchTerm) || kelas.includes(searchTerm) || matakuliah.includes(searchTerm) || dosen.includes(searchTerm) || ruangan.includes(searchTerm);

    const matchesHari = selectedHari?.value ? item.hari === selectedHari.value : true;
    const matchesMatakuliah = selectedMatakuliah?.value ? item.matakuliah?.namamatakuliah === selectedMatakuliah.value : true;
    const matchesDosen = selectedDosen?.value ? item.dosen?.nama === selectedDosen.value : true;

    return matchesSearch && matchesHari && matchesMatakuliah && matchesDosen;
  });
    ...
    <>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap", alignItems: "center" }}>
          <Select options={hariOptions} value={selectedHari} onChange={(selected) => setSelectedHari(selected)} placeholder="Filter Hari" isClearable styles={{ container: (base) => ({ ...base, maxWidth: "100%", fontSize: "0.8rem" }) }} />
          ...
          <Select
            ...
            isClearable
            styles={{ container: (base) => ({ ...base, width: 180, fontSize: "0.8rem" }) }}
          />
          ...
          <Select
            ...
            isClearable
            styles={{ container: (base) => ({ ...base, maxWidth: "100%", fontSize: "0.8rem" }) }}
          />
        </Box>
        <Divider sx={{ mb: 2 }} />
