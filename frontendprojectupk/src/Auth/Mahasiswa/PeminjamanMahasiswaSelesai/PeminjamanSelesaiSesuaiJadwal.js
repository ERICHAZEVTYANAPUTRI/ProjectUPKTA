...
const PeminjamanSelesaiMahasiswaSesuaiJadwal = () => {
  const [selectedHari, setSelectedHari] = useState(""); // untuk filter hari
  const [selectedStatus, setSelectedStatus] = useState(""); // untuk filter status
  ...
  const pageOptions = [
    { value: "/peminjamanselesaisesuaijadwalbaianmahasiswa", label: "Peminjaman Selesai Sesuai jadwal" },
    { value: "/pengembalianselesaimahasiswa", label: "Peminjaman Selesai Tidak SesuaiJadwal" },
  ];
  ...
  useEffect(() => {
    const tahunAjaranUnik = Array.from(
      new Map(
        jadwalMahasiswa
          .filter((item) => item.tahunajaran) // pastikan tahunajaran ada
          .map((item) => [
            item.tahunajaran.id,
            {
              value: item.tahunajaran.id,
              label: `${item.tahunajaran.tahun} - ${item.tahunajaran.semester.toLowerCase()} ${item.tahunajaran.is_aktif === 1 ? "(Aktif)" : "(Nonaktif)"}`,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const jadwalRes = await axios.get("http://localhost:8000/api/peminjamanselesaimahasiswahistory");
        const semuaJadwal = jadwalRes.data;

        const filtered = semuaJadwal.filter((item) => item.mahasiswa_id === user.id);
  ...
  const filteredJadwal = jadwalMahasiswa.filter((item) => {
    ...
    const cocokHari = selectedHari === "" || item.hari?.toLowerCase() === selectedHari.toLowerCase();

    const cocokDosen = selectedDosen === "" || item.dosen?.id === selectedDosen;

    const cocokMatkul = selectedMatkul === "" || item.matakuliah?.id === selectedMatkul;
    const cocokTahunAjaran = selectedTahunAjaran === null || item.tahunajaran_id === selectedTahunAjaran.value;
    ...
    return cocokSearch && cocokHari && cocokStatus && cocokDosen && cocokMatkul && cocokTahunAjaran;
  });
  ...
  const dosenOptions = jadwalMahasiswa.length ? Array.from(new Map(jadwalMahasiswa.filter((item) => item.dosen?.id && item.dosen?.nama).map((item) => [item.dosen.id, { value: item.dosen.id, label: item.dosen.nama }])).values()) : [];

  const matkulOptions = jadwalMahasiswa.length
    ? Array.from(new Map(jadwalMahasiswa.filter((item) => item.matakuliah?.id && item.matakuliah?.namamatakuliah).map((item) => [item.matakuliah.id, { value: item.matakuliah.id, label: item.matakuliah.namamatakuliah }])).values())
    : [];
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: "flex", gap: 2, alignItems: "center", flexWrap: "wrap", mb: 2 }}>
          ...
          <Select
            ...
            onChange={(e) => {
              setSelectedTahunAjaran(e || tahunAjaranAktif); // reset ke aktif kalau clear
            }}
        ...
        <Box sx={{ display: "flex", gap: 2, alignItems: "center", flexWrap: "wrap", mb: 2 }}>
          <Select
            ...
            value={selectedHari ? { label: selectedHari.charAt(0).toUpperCase() + selectedHari.slice(1), value: selectedHari } : null}
            ...
            value={
              selectedStatus
                ? {
                    ...
                    label: selectedStatus === "kosong" ? "Kuliah Kosong" : selectedStatus === "dialihkan" ? "Kuliah Dialihkan" : "Kuliah Dilaksanakan",
                  }
                : null
            }
          ...
          <Select
            ...
            value={dosenOptions.find((opt) => opt.value === selectedDosen) || null}
          ...
          <Select
            ...
            value={matkulOptions.find((opt) => opt.value === selectedMatkul) || null}
                ...
                <TableCell sx={cellStyle}>
                  <Tooltip title="Peminjaman selesai" arrow>
                    <Button
                      ...
                      onClick={() => {
                        if (item.statusdigunakan === "digunakan") {
                          swal({
                        ...
                        } else if (item.statustidakdigunakan === "kosong") {
                          swal({
                            title: "Kuliah Kosong",
                        ...
                        } else if (item.statusdialihkan === "dialihkan") {
                          swal({
