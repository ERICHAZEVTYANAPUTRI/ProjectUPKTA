import { Box, Button, Card, CardContent, InputAdornment, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
const JadwalMahasiswa = () => {
  ...
  const handlePinjam = (jadwal) => {
    ...
    swal({
      ...
      text: `${jadwal.matakuliah?.namamatakuliah || ""} pada hari ${jadwal.hari} jam ${jadwal.jammulai}`,
      icon: "warning",
      ...
      if (willPinjam) {
        try {
          const token = localStorage.getItem("token");
          await axios.put(`http://localhost:8000/api/pinjamjadwal/${jadwal.id}`, { statuspeminjaman: "terpinjam" }, { headers: { Authorization: `Bearer ${token}` } });

          swal("Berhasil!", "Jadwal berhasil dipinjam.", "success");

          setJadwalMahasiswa((prev) => prev.map((j) => (j.id === jadwal.id ? { ...j, statuspeminjaman: "terpinjam" } : j)));
        } catch (error) {
          console.error(error);
  ...
  const jadwalFiltered = jadwalMahasiswa
    ...
    .filter((item) => {
      ...
      return (
        ...
        `${item.jammulai} - ${item.jamselesai}`.toLowerCase().includes(search) ||
    ...
    <div className="dashboard-container">
      ...
      >
        <Card sx={{ maxWidth: 1200, mx: "auto", p: 2 }}>
          <CardContent>
            {/* Search Input with button inside TextField */}
            <Box sx={{ mb: 2, display: "flex", justifyContent: "flex-end" }}>
              <TextField
                ...
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end" sx={{ display: "flex", gap: 1, alignItems: "center", paddingRight: 0 }}>
                      {/* Hapus IconButton */}
                      <Button
                        variant="contained"
                        size="small"
                        onClick={() => {
                          // Opsional: fungsi pencarian manual jika mau
                          // sekarang search sudah live di onChange
                        }}
                        sx={{
                          bgcolor: "#0C20B5",
                          "&:hover": { bgcolor: "#0931a7" },
                          textTransform: "none",
                          minWidth: "60px",
                          height: "30px",
                          display: "flex",
                          ...
                          padding: "6px", // sesuaikan padding agar icon pas
                        }}
                      >
                        <BsSearch />
                      </Button>
              ...
              <TableContainer sx={{ border: "1px solid #e5e6ed", borderRadius: 1 }}>
                <Table>
                  <TableHead sx={{ backgroundColor: "#3449e4" }}>
                    <TableRow>
                      {["No", "Hari", "Jam", "Ruangan", "Mata Kuliah", "Dosen", "Aksi", "Pinjam"].map((head, i, arr) => (
                        <TableCell
                          key={head}
                          align="center"
                          sx={{
                  ...
                  <TableBody>
                    {jadwalFiltered.map((item, index) => (
                      <TableRow key={item.id} hover>
                        ...
                        <TableCell align="center">{item.ruangan?.name || "-"}</TableCell>
                        <TableCell align="center">{item.matakuliah?.namamatakuliah || "-"}</TableCell>
                        <TableCell align="center">{item.dosen?.nama || "-"}</TableCell>
                        <TableCell align="center">
                          <Button size="small" variant="outlined" onClick={() => handleDetail(item.id)}>
                            Detail
                          </Button>
                        </TableCell>
                        <TableCell align="center">
                          <Button size="small" variant="outlined" color="success" onClick={() => handlePinjam(item)} disabled={item.statuspeminjaman === "terpinjam"}>
                            {item.statuspeminjaman === "terpinjam" ? "Terpinjam" : "Pinjam"}
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}
