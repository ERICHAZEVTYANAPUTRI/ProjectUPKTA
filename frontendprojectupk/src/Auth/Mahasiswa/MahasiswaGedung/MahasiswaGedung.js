    ...
    <>
      ...
      <Box sx={{ backgroundColor: "#f0f4ff", minHeight: "100%", pt: 3 }}>
        ...
        >
          <Box
            sx={{
              background: "linear-gradient(90deg, #1446a0, #1e88e5)",
              px: { xs: 2, sm: 4 },
              py: 0.8,
              borderRadius: "12px",
        ...
        >
          {loading && Array.from(new Array(6)).map((_, idx) => <GedungCardSkeleton key={idx} />)}
          ...
          {!loading &&
            ...
            gedungs.map((gedung) => (
              <GedungCard
                key={gedung.id}
                gedung={gedung}
                onClick={() => handleMoreInfo(gedung.id)} // <-- Di sini handler navigasi dipasang
              />
            ))}
