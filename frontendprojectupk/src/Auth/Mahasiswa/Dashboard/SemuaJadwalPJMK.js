...
const JadwalkelasPJMK = ({ open, onJadwalDipinjam }) => {
  ...
  const [kelasKosongHariIni, setKelasKosongHariIni] = useState(() => loadFromLocalStorage(LS_KEY_KELAS_KOSONG));
  const [kelasDialihkanHariIni, setKelasDialihkanHariIni] = useState(() => loadFromLocalStorage(LS_KEY_KELAS_DIALIHKAN));
  ...
  useEffect(() => {
    localStorage.setItem(LS_KEY_KELAS_KOSONG, JSON.stringify({ date: getTodayString(), ids: kelasKosongHariIni }));
  }, [kelasKosongHariIni]);

  useEffect(() => {
    localStorage.setItem(LS_KEY_KELAS_DIALIHKAN, JSON.stringify({ date: getTodayString(), ids: kelasDialihkanHariIni }));
  ...
  const isTombolDialihkanAktif = (item) => {
    ...
    const jamMulai = getDateTimeFromTime(item.jammulai);
    const jamSelesai = getDateTimeFromTime(item.jamselesai);
    const limaMenitSebelum = new Date(jamMulai.getTime() - 5 * 60 * 1000);

    return hariSekarang === hariJadwal && now >= limaMenitSebelum && now <= jamSelesai && !kelasKosongHariIni.includes(item.id) && !kelasDialihkanHariIni.includes(item.id);
  };
  ...
  const isTombolKelasKosongAktif = (item) => {
    ...
    return (
      hariSekarang === hariJadwal && now >= limaMenitSebelum && now <= jamSelesai && !kelasKosongHariIni.includes(item.id) && !kelasDialihkanHariIni.includes(item.id) // ⬅️ tambahkan ini
    );
  };
  useEffect(() => {
    const interval = setInterval(() => {
      const today = getTodayString();
      const kosongData = JSON.parse(localStorage.getItem(LS_KEY_KELAS_KOSONG));
      const dialihkanData = JSON.parse(localStorage.getItem(LS_KEY_KELAS_DIALIHKAN));
  ...
  return (
    ...
    >
      ...
      >
        {selectedJadwal && isTombolDialihkanAktif(selectedJadwal) && selectedJadwal.statuspeminjaman !== "dipinjam" && selectedJadwal.statuspeminjaman !== "prosespengembalian" && (
          <MenuItem
            onClick={async () => {
              handleCloseMenu();
              ...
              if (confirm) {
                try {
                  // Panggil API untuk menyimpan ke peminjamanselesais
                  await axios.post(
                  ...
                  // Tambahkan ke state lokal agar tombol hilang
                  setKelasDialihkanHariIni((prev) => [...prev, selectedJadwal.id]);

                  swal("Berhasil", "Kelas ditandai sebagai dialihkan.", "success");
                } catch (error) {
                  console.error(error);
                  swal("Gagal", "Tidak bisa tandai kelas sebagai dialihkan.", "error");
                }
              }
            }}
            sx={{
        ...
        {selectedJadwal && isTombolKelasKosongAktif(selectedJadwal) && selectedJadwal.statuspeminjaman !== "dipinjam" && selectedJadwal.statuspeminjaman !== "prosespengembalian" && (
          <MenuItem
            onClick={() => {
              handleCloseMenu();
              swal({
                ...
                if (willDo) {
                  try {
                    ...
                    setKelasKosongHariIni((prev) => [...prev, selectedJadwal.id]);
                    swal("Berhasil", "Kelas ditandai kosong.", "success");
                  } catch (err) {
                    swal("Gagal", "Tidak bisa tandai kelas kosong.", "error");
                  }
                }
              });
            }}
