...
const PengajuanPeminjamaDiterimaCard = ({ open, onJadwalDipinjam }) => {
    ...
    const canShowPinjamButton = (item) => {
        ...
        const itemDay = item.hari?.toLowerCase();
        const itemDate = item.tanggal;

        const selesaiDateTime = new Date(`${itemDate}T${item.jamselesai}`);
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get(
                "http://localhost:8000/api/pengajuan-diterima",
                {
    ...
    const handlePinjam = async () => {
        ...
        if (confirm) {
            try {
                await axios.post(
                    `http://localhost:8000/api/pengajuan-pinjamsaya/${selectedId}`,
                    null,
                    {
                        headers: { Authorization: `Bearer ${token}` },
                    }
    ...
    return (
        ...
        >
            ...
            >
                ...
                {selectedPengajuan &&
                    canShowPinjamButton(selectedPengajuan) && (
                        <MenuItem
