...
const TambahPenjadwalGanjilAdminJurusan = () => {
  ...
  const handleSelectChange = (selectedOption, index) => {
    ...
    // Lakukan filter matakuliah berdasarkan semester mahasiswa
  ...
  useEffect(() => {
    const fetchTahunAjaran = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/tahunajaranganjil/${tahunAjaranganjilId}`);
        if (response.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log({
      tahunajaranganjil_id: tahunAjaranganjilId,
      jadwal: formattedJadwal,
    });

    try {
      await axios.post("http://localhost:8000/api/penjadwalanganjil", {
        tahunajaranganjil_id: tahunAjaranganjilId,
        jadwal: formattedJadwal,
      });
  ...
  return (
    <div>
      ...
      <div style={{ marginLeft: "20px", marginTop: "20px" }}>
        <h2>Tambah Jadwal Kuliah Semester Ganjil</h2>
      </div>
...
export default TambahPenjadwalGanjilAdminJurusan;
