...
import { Box, Button, DialogActions, DialogContent, Divider, Grid, MenuItem, Modal, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Tooltip, Typography } from "@mui/material";
import { styled } from "@mui/material/styles";
import axios from "axios";
import { useEffect, useState } from "react";
import { BsFolderPlus, BsPencil, BsSearch, BsTrash } from "react-icons/bs";
...
const TabelProdi = () => {
  ...
  const handleSort = (column) => {
    ...
    const sorted = [...prodiList].sort((a, b) => {
  ...
  useEffect(() => {
    const fetchUserAndProdi = async () => {
      try {
        ...
        const prodiResponse = await axios.get("http://localhost:8000/api/prodi", {
          headers: { Authorization: `Bearer ${token}` },
        });

        const filtered = prodiResponse.data.filter((prodi) => prodi.adminjurusan?.kodejurusan === userData.kodejurusan);
  ...
  const handleUpdate = (prodi) => {
    openEditModal(prodi);
  };

  const filteredData = prodiList.filter((prodi) => {
    const term = searchTerm.toLowerCase();
    return prodi.namaprodi?.toLowerCase().includes(term) || prodi.kodeprodi?.toLowerCase().includes(term);
  });
  ...
  const handleSaveProdi = async (data, id = null) => {
    try {
      const token = localStorage.getItem("token");

      if (id) {
        await axios.put(`http://localhost:8000/api/prodi/${id}`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data prodi berhasil diperbarui.", "success");
      } else {
        data.kodejurusan = user.kodejurusan;
        data.id_jurusan = user.id_jurusan;
        data.adminjurusan_id = user.id;

        await axios.post(`http://localhost:8000/api/prodi`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data prodi berhasil ditambahkan.", "success");
      }
      ...
      const filtered = prodiResponse.data.filter((prodi) => prodi.adminjurusan?.kodejurusan === user.kodejurusan);
      setProdiList(filtered);
      closeModal();
    } catch (error) {
    ...
    swal({
      ...
      if (willDelete) {
        try {
          ...
          await axios.delete(`http://localhost:8000/api/prodi/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });

          setProdiList((prev) => prev.filter((prodi) => prodi.id !== id));
          swal("Terhapus!", "Data prodi berhasil dihapus.", "success");
        } catch (error) {
  ...
  return (
    <>
      <ModalProdi open={modalOpen} handleClose={closeModal} onSave={handleSaveProdi} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 1 }}>
          ...
          >
            <BsFolderPlus style={{ fontSize: "18px" }} />
            Tambah Prodi
          </Button>
        </Box>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 2 }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, marginBottom: "-10px" }}>
            ...
            <Select
              ...
              sx={{ fontSize: "12px", height: "25px", width: "60px" }} // batasi lebar Select utama
              MenuProps={{
                PaperProps: {
                  sx: {
                    maxHeight: 120,
                    width: "60px", // pastikan dropdownnya juga sempit
                    mt: 0.5,
                    overflowY: "auto",
                    // 👇 Sembunyikan scroll bar (semua browser modern)
              ...
              {[5, 10, 25].map((value) => (
                <MenuItem
                  ...
                  sx={{
                    ...
                    py: 0.5,
                    minHeight: "unset",
                    justifyContent: "center", // ⬅️ teks berada di tengah
                    textAlign: "center", // ⬅️ pastikan teks rata tengah
                  }}
                >
                  {value}
                </MenuItem>
      ...
      >
        <Table aria-label="Data Prodi" sx={{ width: "100%", minWidth: 600, tableLayout: "auto" }}>
          <TableHead sx={{ backgroundColor: "#0C20B5" }}>
            <TableRow>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "kodeprodi"} direction={order} onClick={() => handleSort("kodeprodi")} label="Kode Prodi" />
              </TableCell>
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "namaprodi"} direction={order} onClick={() => handleSort("namaprodi")} label="Nama Prodi" />
              </TableCell>
              <TableCell sx={headStyle}>
          ...
          <TableBody>
            {displayedItems.map((prodi, index) => (
              <TableRow key={prodi.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#F6F6F6" : "#FFFFFF" }}>
                <TableCell sx={cellStyle}>{index + 1 + currentPage * rowsPerPage}</TableCell>
                <TableCell sx={cellStyle}>{prodi.kodeprodi}</TableCell>
                <TableCell sx={cellStyle}>{prodi.namaprodi}</TableCell>
...
const ModalProdi = ({ open, handleClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        kodeprodi: initialData.kodeprodi,
        namaprodi: initialData.namaprodi,
      });
    } else {
      setFormData({ kodeprodi: "", namaprodi: "" });
    }
  }, [initialData]);
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      ...
      >
        ...
        >
          {initialData ? "Edit Prodi" : "Tambah Prodi"}
        </Typography>
