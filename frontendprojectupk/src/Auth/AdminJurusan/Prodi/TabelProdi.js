...
const TabelProdi = () => {
    ...
    useEffect(() => {
        const fetchUserAndProdi = async () => {
            try {
                ...
                const prodiResponse = await axios.get(
                ...
                const filtered = prodiResponse.data.filter(
                    (prodi) =>
                        prodi.adminjurusan?.kodejurusan === userData.kodejurusan
                );
    ...
    const filteredData = prodiList.filter((prodi) => {
        ...
        return (
            prodi.namaprodi?.toLowerCase().includes(term) ||
            prodi.kodeprodi?.toLowerCase().includes(term)
        );
    });
    ...
    const handleSaveProdi = async (data, id = null) => {
        try {
            ...
            const filtered = prodiResponse.data.filter(
                (prodi) => prodi.adminjurusan?.kodejurusan === user.kodejurusan
            );
            setProdiList(filtered);
            closeModal();
        } catch (error) {
                    ...
                    <TableHead sx={{ backgroundColor: "#0C20B5" }}>
                        <TableRow>
                            ...
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "kodeprodi"}
                                    direction={order}
                                    onClick={() => handleSort("kodeprodi")}
                                    label="Kode Prodi"
                                />
                            </TableCell>
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "namaprodi"}
                                    direction={order}
                                    onClick={() => handleSort("namaprodi")}
                                    label="Nama Prodi"
                                />
                            </TableCell>
                            <TableCell sx={headStyle}>
