...
const DataTahunAjaran = () => {
  ...
  const handleToggleStatus = async (ta) => {
    ...
    try {
      ...
      setTahunAjaranList((prev) => prev.map((item) => (item.id === ta.id ? { ...item, is_aktif: newStatus } : item)));
    } catch (error) {
  ...
  useEffect(() => {
    const fetchUserAndTahunAjaran = async () => {
      try {
        ...
        const tahunajaranResponse = await axios.get("http://localhost:8000/api/tahunajaran", {
          headers: { Authorization: `Bearer ${token}` },
        });

        const filtered = tahunajaranResponse.data.filter((ta) => ta.adminjurusan?.kodejurusan === userData.kodejurusan);
  ...
  const filteredData = tahunAjaranList.filter(
    (ta) => (ta.tahun || "").toLowerCase().includes(searchTerm.toLowerCase()) || (ta.semester || "").toLowerCase().includes(searchTerm.toLowerCase()) || (ta.adminjurusan?.nama_lengkap || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleSaveTahunAjaran = async (data, id = null) => {
    try {
      ...
      if (!data.tahun || !data.semester) {
        swal("Gagal!", "Tahun dan semester tidak boleh kosong.", "warning");
        return;
      }

      if (id) {
        await axios.put(`http://localhost:8000/api/tahunajaran/${id}`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data tahun ajaran berhasil diperbarui.", "success");
      } else {
        data.kodejurusan = user.kodejurusan;
        data.id_jurusan = user.id_jurusan;
        data.adminjurusan_id = user.id;

        await axios.post(`http://localhost:8000/api/tahunajaran`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data tahun ajaran berhasil ditambahkan.", "success");
      }
      ...
      const filtered = response.data.filter((ta) => ta.adminjurusan?.kodejurusan === user.kodejurusan);
      setTahunAjaranList(filtered);
      closeModal();
    } catch (error) {
    ...
    swal({
      ...
      if (willDelete) {
        try {
          ...
          await axios.delete(`http://localhost:8000/api/tahunajaran/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          setTahunAjaranList((prev) => prev.filter((ta) => ta.id !== id));

          swal("Terhapus!", "Data tahunajaran berhasil dihapus.", "success");
        } catch (error) {
  ...
  return (
    <>
      <ModalTahunajaran open={modalOpen} handleClose={closeModal} onSave={handleSaveTahunAjaran} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
      ...
      >
        <Table aria-label="Data Tahun Ajaran" sx={{ width: "100%", minWidth: 600, tableLayout: "auto" }}>
          <TableHead sx={{ backgroundColor: "#0C20B5" }}>
            <TableRow>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "tahun"} direction={order} onClick={() => handleSort("tahun")} label="Tahun Ajaran" />
              </TableCell>
              <TableCell sx={headStyle}>
          ...
          <TableBody>
            {displayedItems.map((ta, index) => (
              <TableRow key={ta.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#F6F6F6" : "#FFFFFF" }}>
                <TableCell sx={cellStyle}>{index + 1 + currentPage * rowsPerPage}</TableCell>
                <TableCell sx={cellStyle}>{ta.tahun}</TableCell>
                <TableCell sx={cellStyle}>{ta.semester}</TableCell>
                <TableCell sx={cellStyle}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <IconButton
                      onClick={() => handleToggleStatus(ta)}
                      sx={{
                        color: ta.is_aktif ? "success.main" : "grey.500",
                        transition: "all 0.3s ease",
                        transform: ta.is_aktif ? "scale(1.1)" : "scale(1)",
                        "&:hover": {
                          transform: "scale(1.2)",
                          color: ta.is_aktif ? "success.dark" : "grey.700",
                        },
                      }}
                    >
                      {ta.is_aktif ? <ToggleOnIcon fontSize="medium" /> : <ToggleOffIcon fontSize="medium" />}
                    </IconButton>
...
const ModalTahunajaran = ({ open, handleClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        tahun: tahunOptions.find((opt) => opt.value === initialData.tahun) || null,
        semester: semesterOptions.find((opt) => opt.value === initialData.semester.toLowerCase()) || null,
      });
    } else {
      setFormData({
        tahun: null,
  ...
  const handleSubmit = () => {
    // Kirim hanya value dari option yang dipilih
    onSave(
      {
      ...
      >
        ...
        >
          <Box sx={{ marginBottom: "15px", marginTop: "-5px" }}>
            <Typography variant="body2" sx={{ marginBottom: "1px", color: "#333" }}>
              Tahun Ajaran
            </Typography>
