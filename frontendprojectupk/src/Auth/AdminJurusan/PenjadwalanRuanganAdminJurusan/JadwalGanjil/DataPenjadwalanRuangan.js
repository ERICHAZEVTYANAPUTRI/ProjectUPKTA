...
const PenjadwalanRuanganPerkuliahan = () => {
    ...
    useEffect(() => {
        const fetchTahunAjaran = async () => {
            try {
                ...
                const dataByAdmin = res.data.filter(
                    (item) => item.adminjurusan_id === user.id
                );
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                const [prodiRes, kelasRes, matkulRes, dosenRes] =
                    await Promise.all([
                ...
                setProdiList(
                    prodiRes.data.filter(
                        (item) => item.adminjurusan_id === user.id
                    )
                );
                setKelasList(
                    kelasRes.data.filter(
                        (item) => item.adminjurusan_id === user.id
                    )
                );
                setMatakuliahList(
                    matkulRes.data.filter(
                        (item) => item.adminjurusan_id === user.id
                    )
                );
                setDosenList(
                    dosenRes.data.filter(
                        (item) => item.adminjurusan_id === user.id
                    )
                );
            } catch (err) {
    ...
    const user = JSON.parse(localStorage.getItem("user"));
    const hasJadwalBelumTerkirim = penjadwalanList.some(
        (jadwal) =>
            jadwal.statusterkirim === "belumterkirim" &&
            jadwal.adminjurusan_id === user.id
    );
    ...
    const adaJadwalBelumTerkirim = penjadwalanList.some(
        (item) => item.statusterkirim !== "terkirim"
    );
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const user = JSON.parse(localStorage.getItem("user"));
                const res = await axios.get(
                    "http://localhost:8000/api/penjadwalanruanganget"
                );

                const filtered = res.data.filter(
                    (item) =>
                        item.adminjurusan_id === user.id &&
                        item.tahunajaran?.id === tahunAjaranId
                );
        ...
        swal({
            ...
            if (willSend) {
                try {
                    ...
                    const user = JSON.parse(localStorage.getItem("user"));
                    const semuaJadwal = res.data.filter(
                        (item) =>
                            item.adminjurusan_id === user.id &&
                            item.statusterkirim === "belumterkirim"
                    );

                    const jadwalDenganRuangan = semuaJadwal.filter(
                        (item) => item.ruangan_id
                    );
                    const jadwalTanpaRuangan = semuaJadwal.filter(
                        (item) => !item.ruangan_id
                    );
                    ...
                    const pesanGagal =
                        jadwalTanpaRuangan.length > 0
                            ? `${jadwalTanpaRuangan.length} jadwal gagal dikirim karena belum memiliki ruangan.`
                            : "";
                    ...
                    const filtered = refresh.data.filter(
                        (item) => item.adminjurusan_id === user.id
                    );
                    setPenjadwalanList(filtered);
                } catch (error) {
    ...
    const handleDeleteClick = async (id) => {
        swal({
            ...
            if (willDelete) {
                try {
                    await axios.delete(
                        `http://localhost:8000/api/penjadwalanruangandelete/${id}`
                    );
                    setPenjadwalanList((prev) =>
    ...
    const filteredPenjadwalan = penjadwalanList.filter((item) => {
        ...
        const matchKelas = selectedKelas
            ...
            : true;
        const matchMatakuliah = selectedMatakuliah
            ? item.matakuliah?.id === selectedMatakuliah.value
            : true;
        const matchDosen = selectedDosen
    ...
    const displayedItems = filteredPenjadwalan.slice(
                    ...
                    <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
                        <Select
                            ...
                            value={
                                selectedTahunAjaran ||
                                (() => {
                                    const aktif = tahunAjaranList.find(
                        ...
                        {hasJadwalBelumTerkirim && (
                            <Button
                                ...
                                sx={{
                                    ...
                                    textTransform: "none",
                                    background:
                                        "linear-gradient(135deg, #FFA726, #FFEB3B)",
                                    color: "#000",
                                    boxShadow:
                                    ...
                                    "&:hover": {
                                        background:
                                            "linear-gradient(135deg, #FB8C00, #FFF176)",
                                        transform: "translateY(-1px)",
                                        boxShadow:
                                            "0 6px 14px rgba(255, 193, 7, 0.5)",
                                    },
                                }}
                            >
                        ...
                        <Button
                            ...
                            sx={{
                                ...
                                "&:hover": {
                                    ...
                                    boxShadow:
                                        "0 6px 14px rgba(21, 101, 42, 0.5)",
                                    transform: "translateY(-2px)",
                                },
                                "&:active": {
                                    transform: "scale(0.97)",
                                    boxShadow:
                                        "0 2px 6px rgba(21, 101, 42, 0.4)",
                                },
                            }}
                        >
                    ...
                    <TableBody>
                        {displayedItems.map((item, index) => (
                            ...
                            >
                                ...
                                <TableCell sx={cellStyle}>
                                    {item.ruangan?.name || "Belum Ditentukan"}
                                </TableCell>
                                <TableCell sx={cellStyle}>
                                    {item.kelas?.nama || "-"}
                                </TableCell>
