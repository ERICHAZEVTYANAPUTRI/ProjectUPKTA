...
const EditPenjadwalanRuanganPerkuliahan = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [res, prodi, thn, mk, dosen, mhs, kelas] =
                    await Promise.all([
                        axios.get(
                            `http://localhost:8000/api/penjadwalanruangan/${id}`
                        ),
                ...
                const filteredProdi = prodi.data.filter(
                ...
                setTahunAjaranList(
                    thn.data
                        .filter(
                            (t) =>
                                t.adminjurusan_id === adminJurusanId &&
                                t.is_aktif === 1
                        )
                        .map((t) => ({
                            value: t.id,
                ...
                const filteredMatakuliah = mk.data.filter(
                    (mkItem) =>
                        mkItem.adminjurusan_id === adminJurusanId &&
                        filteredProdi.some((p) => p.id === mkItem.prodi_id)
                );
    ...
    useEffect(() => {
        ...
        const tahunAjaranTerpilih = tahunAjaranList.find(
            (t) => t.value === data.tahunajaran_id
        );
                    ...
                    >
                        <FormControl fullWidth sx={{ mb: 2 }}>
                            ...
                            <Select
                                ...
                                onChange={(val) =>
                                    handleSelect(val, "tahunajaran_id")
                                }
                                isClearable
                            ...
                            >
                                ...
                                <FormControl fullWidth sx={{ flex: 2 }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={(val) =>
                                            handleSelect(val, "kodematakuliah")
                                        }
                                        isClearable
                                        menuPortalTarget={document.body}
                                        styles={{
