...
const TambahPenjadwalanRuanganPerkuliahan = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [mhsRes, mkRes, prodiRes, thnRes, dosenRes, kelasRes] =
                    await Promise.all([
                ...
                const filteredProdi = prodiRes.data.filter(
                    (p) => p.adminjurusan_id === adminJurusanId
                );
                const filteredTahun = thnRes.data.filter(
                    (t) => t.adminjurusan_id === adminJurusanId
                );
                const filteredMatakuliah = mkRes.data.filter(
                    (mk) =>
                        mk.adminjurusan_id === adminJurusanId &&
                        filteredProdi.some((p) => p.id === mk.prodi_id)
                );
                const filteredMahasiswa = mhsRes.data.filter((mhs) =>
                    filteredProdi.some((p) => p.id === mhs.prodi_id)
                );
                const filteredDosen = dosenRes.data.filter(
                    (dosen) => dosen.adminjurusan_id === adminJurusanId
                );
                const filteredKelas = kelasRes.data.filter(
                    (k) => k.adminjurusan_id === adminJurusanId
                );
    ...
    useEffect(() => {
        ...
        const filtered = allMahasiswa.filter((mhs) => {
            const isSameProdi =
                Number(mhs.prodi_id) === Number(selectedProdi?.value);
            const isSameKelas =
                Number(mhs.kelas_id) === Number(selectedKelas?.value);
            ...
            const cocokSemester = isSemesterGanjil
                ? semesterMahasiswa % 2 === 1
                : isSemesterGenap
                ? semesterMahasiswa % 2 === 0
                : true; // fallback
        ...
        setMahasiswaList(
            filtered.map((mhs) => ({
                value: mhs.id,
                label: `${mhs.nama_lengkap}_${mhs.nim}_${getNamaKelas(
                    mhs.kelas_id
                )}`,
            }))
        );
    ...
    const handleSubmit = async (e) => {
        ...
        const isFormValid = jadwalList.every(
            (j) =>
                j.mahasiswa_id &&
                j.dosen_id &&
                j.hari &&
        ...
        if (
            ...
            swal(
                "Peringatan",
                "Mohon lengkapi semua field sebelum menyimpan.",
                "warning"
            );
            return;
        }
        ...
        try {
            const res = await axios.post(
                "http://localhost:8000/api/cek-bentrok-jadwal-form",
                {
            ...
            if (res.data.bentrok.length > 0) {
                const list = res.data.bentrok
                    .map((i) => `Jadwal ke-${i + 1}`)
                    .join(", ");
                swal(
                    "Jadwal Bentrok",
                    `Formulir ${list}, kelas sudah memiliki jadwal kuliah lain di hari dan jam yang sama. Harap perbaiki.`,
                    "error"
                );
                return;
            }
            ...
            if (
                ...
                const formatted = Object.entries(errorList)
                    .map(
                        ([index, messages]) =>
                            `Jadwal ke-${parseInt(index) + 1}: ${messages.join(
                                ", "
                            )}`
                    )
                    .join("\n");
    ...
    const [tempJamselesaiList, setTempJamselesaiList] = useState(
        jadwalList.map((item) => parseTimeString(item.jamselesai) || null)
    );
                ...
                <Box sx={{ flex: 1, minWidth: 280 }}>
                    ...
                    >
                        <FormControl fullWidth sx={{ mb: 2 }}>
                            ...
                            <Select
                                options={tahunAjaranList.filter(
                                    (item) => item.is_aktif === 1
                                )}
                        ...
                        {selectedProdi && (
                            <FormControl fullWidth>
                                ...
                                <Select
                                    options={kelasList.filter(
                                        (k) =>
                                            k.prodi_id === selectedProdi.value
                                    )}
                                    value={selectedKelas}
