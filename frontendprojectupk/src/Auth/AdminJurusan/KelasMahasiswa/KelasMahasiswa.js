...
import { BsPencil, BsPersonLinesFill, BsSearch, BsTrash } from "react-icons/bs";
...
const KelasMahasiswa = () => {
  ...
  useEffect(() => {
    const fetchKelas = async () => {
      try {
        ...
        const kelasResponse = await axios.get("http://localhost:8000/api/kelasmahasiswa", {
          headers: { Authorization: `Bearer ${token}` },
        });

        const filtered = kelasResponse.data.filter((kls) => kls.adminjurusan?.kodejurusan === userData.kodejurusan);
  ...
  const getNamaProdi = (idProdi) => {
    const prodi = prodiList.find((p) => p.id === idProdi);
  ...
  const handleUpdate = (kls) => {
    openEditModal(kls);
  };
  const filteredData = kelasList.filter((kls) => {
    const term = searchTerm.toLowerCase();

    return kls.nama?.toLowerCase().includes(term) || kls.prodi?.namaprodi?.toLowerCase().includes(term);
  });
  ...
  const handleSaveKelas = async (data, id = null) => {
    try {
      const token = localStorage.getItem("token");

      if (id) {
        await axios.put(`http://localhost:8000/api/kelasmahasiswa/edit/${id}`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data kelas berhasil diperbarui.", "success");
      } else {
        data.kodejurusan = user.kodejurusan;
        data.id_jurusan = user.id_jurusan;
        data.adminjurusan_id = user.id;

        await axios.post(`http://localhost:8000/api/kelasmahasiswa/tambah`, data, {
          headers: { Authorization: `Bearer ${token}` },
        });
        swal("Berhasil!", "Data kelas berhasil ditambahkan.", "success");
      }
      ...
      const filtered = kelasResponse.data.filter((kls) => kls.adminjurusan?.kodejurusan === user.kodejurusan);
      setKelasList(filtered);
      closeModal();
    } catch (error) {
    ...
    swal({
      ...
      if (willDelete) {
        try {
          ...
          setKelasList((prev) => prev.filter((kls) => kls.id !== id));
          swal("Terhapus!", "Data kelas berhasil dihapus.", "success");
        } catch (error) {
  ...
  return (
    <>
      <ModalKelas open={modalOpen} handleClose={closeModal} onSave={handleSaveKelas} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 1 }}>
          ...
          >
            <BsPersonLinesFill style={{ fontSize: "18px" }} />
            Tambah Kelas
          </Button>
        </Box>
      ...
      >
        <Table aria-label="Data Kelas" sx={{ width: "100%", minWidth: 600, tableLayout: "auto" }}>
          <TableHead sx={{ backgroundColor: "#0C20B5" }}>
            <TableRow>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "nama"} direction={order} onClick={() => handleSort("nama")} label="Kelas" />
              </TableCell>
              <TableCell sx={headStyle}>
          ...
          <TableBody>
            {displayedItems.map((kls, index) => (
              <TableRow key={kls.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#f5f5f5" : "#ffffff" }}>
                <TableCell sx={cellStyle}>{index + 1 + currentPage * rowsPerPage}</TableCell>
                <TableCell sx={cellStyle}>{kls.nama}</TableCell>
                <TableCell sx={cellStyle}>{getNamaProdi(kls.prodi_id)}</TableCell>
                <TableCell sx={cellStyle}>
                  <Box sx={{ display: "flex", gap: 1, justifyContent: "center" }}>
                    <Tooltip title="Edit data" arrow>
                      <Box
                        onClick={() => handleUpdate(kls)}
                        sx={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: 0.5,
...
const ModalKelas = ({ open, handleClose, onSave, initialData }) => {
