...
import { Box, Button, DialogActions, DialogContent, Divider, Grid, MenuItem, Modal, Select as MuiSelect, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Tooltip, Typography } from "@mui/material";
...
const DataDosen = () => {
  ...
  const [currentPage, setCurrentPage] = useState(0); // <- mulai dari 0
  ...
  useEffect(() => {
    const fetchDosen = async () => {
      try {
        ...
        const dosenResponse = await axios.get("http://localhost:8000/api/dosen", {
          headers: { Authorization: `Bearer ${token}` },
        });

        const filtered = dosenResponse.data.filter((d) => d.adminjurusan?.kodejurusan === userData.kodejurusan);
  ...
  const handleSaveDosen = async (data, id = null) => {
    try {
      ...
      if (id) {
        ...
        swal("Berhasil!", "Data dosen berhasil ditambahkan.", "success");
      }
      ...
      const filtered = dosenResponse.data.filter((d) => d.adminjurusan?.kodejurusan === user.kodejurusan);
      setDosenList(filtered);
      closeModal();
    } catch (error) {
    ...
    swal({
      ...
      if (willDelete) {
        try {
          ...
          setDosenList((prevDosen) => prevDosen.filter((d) => d.id !== id));
          swal("Terhapus!", "Data dosen berhasil dihapus.", "success");
        } catch (error) {
  ...
  return (
    <>
      <ModalDosen open={modalOpen} handleClose={closeModal} onSave={handleSaveDosen} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
      ...
      >
        <Table aria-label="Data Dosen" sx={{ width: "100%", minWidth: 600, tableLayout: "auto" }}>
          <TableHead sx={{ backgroundColor: "#0C20B5" }}>
            <TableRow>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "nip"} direction={order} onClick={() => handleSort("nip")} label="Nip / Nik / Nippppk" />
              </TableCell>
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "prodi_id"} direction={order} onClick={() => handleSort("prodi_id")} label="Prodi" />
              </TableCell>
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "notelpon"} direction={order} onClick={() => handleSort("notelpon")} label="No.Telpon" />
              </TableCell>
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "email"} direction={order} onClick={() => handleSort("email")} label="Email" />
              </TableCell>
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "jabatanfungsional"} direction={order} onClick={() => handleSort("jabatanfungsional")} label="Jabatan" />
              </TableCell>
          ...
          <TableBody>
            {displayedItems.map((d, index) => (
              <TableRow key={d.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#f5f5f5" : "#ffffff" }}>
                ...
                <TableCell sx={cellStyle}>{d.prodi?.namaprodi || "-"}</TableCell>
                <TableCell sx={cellStyle}>{d.notelpon}</TableCell>
                <TableCell sx={cellStyle}>{d.email || "-"}</TableCell>
                <TableCell sx={cellStyle}>{d.jabatanfungsional}</TableCell>
...
const ModalDosen = ({ open, handleClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ nama: "", prodi_id: "", nip: "", notelpon: "", email: "", jabatanfungsional: "" });
    }
  }, [initialData]);
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setProdiOptions(res.data); // Tampilkan semua prodi
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    // 1. Prodi milik adminjurusan yang sedang login
    const prodiSaya = prodiOptions.filter((item) => item.adminjurusan_id === authUser.adminjurusan_id);
    ...
    // 2. Prodi dari jurusan lain
    const prodiLain = prodiOptions.filter((item) => item.adminjurusan_id !== authUser.adminjurusan_id);
        ...
        >
          ...
          <Box sx={{ marginBottom: "15px" }}>
            ...
            <Select
              ...
              value={groupedProdiOptions.flatMap((group) => group.options).find((opt) => opt.value === formData.prodi_id)}
