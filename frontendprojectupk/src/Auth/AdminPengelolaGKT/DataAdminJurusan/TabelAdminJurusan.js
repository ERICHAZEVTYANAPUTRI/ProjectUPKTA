...
const TabelAdminJurusan = () => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(
                    "http://localhost:8000/api/useradminjurusan"
                );
                const filteredData = response.data.filter(
                    (user) => user.role === "admin_jurusan"
                );
    ...
    const handleDelete = async (id) => {
        swal({
            ...
            if (willDelete) {
                try {
                    await axios.delete(
                        `http://localhost:8000/api/useradminjurusan/${id}`
                    );
                    setData(data.filter((user) => user.id !== id));
                    swal(
                        "Berhasil!",
                        "Admin Jurusan Telah Dihapus.",
                        "success"
                    );
                } catch (error) {
    ...
    return (
        <>
            ...
            >
                <Table
                    aria-label="Tabel Admin Jurusan"
                        ...
                        {displayedItems.length > 0 ? (
                            displayedItems.map((admin_jurusan, index) => (
                                ...
                                >
                                    ...
                                    <TableCell sx={cellStyle}>
                                        {admin_jurusan.username}
                                    </TableCell>
                                    <TableCell sx={cellStyle}>
                                        {admin_jurusan.nama_lengkap}
                                    </TableCell>
                                    <TableCell sx={cellStyle}>
                                        {admin_jurusan.nip_nik_nipppk}
                                    </TableCell>
                                    <TableCell sx={cellStyle}>
                                        {admin_jurusan.jabatan}
                                    </TableCell>
                                    <TableCell sx={cellStyle}>
