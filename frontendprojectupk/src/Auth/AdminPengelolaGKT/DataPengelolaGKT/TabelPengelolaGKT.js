...
const TabelPengelolaGKT = ({ fetchData }) => {
    ...
    const fetchPengelolaById = async (id) => {
        try {
            const response = await axios.get(
                `http://localhost:8000/api/userpengelola/${id}`
            );
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(
                    "http://localhost:8000/api/userpengelola"
                );
                const filteredData = response.data.filter(
                    (user) => user.role === "pengelola_gkt"
                );
    ...
    useEffect(() => {
        if (openModal && editPengelola) {
            fetchPengelolaById(editPengelola.id);
        } else {
            setFormData({
                username: "",
                nama_lengkap: "",
    ...
    }, [openModal, editPengelola]);

    const handleEdit = (pengelola) => {
        setEditPengelola(pengelola);
        setFormData({
    ...
    const validateForm = () => {
        ...
        if (!formData.nip_nik_nipppk)
            errors.nip_nik_nipppk = "Kolom wajib diisi!";
        ...
        try {
            ...
            if (editPengelola) {
                response = await axios.put(
                    `http://localhost:8000/api/userpengelola/${editPengelola.id}`,
                    formData
                );
                const updatedPengelola = response.data.data;
                console.log("Updated Pengelola:", updatedPengelola);
                setData((prevData) => {
                    return prevData.map((item) =>
                        item.id === updatedPengelola.id
                            ? updatedPengelola
                            : item
                    );
                });
    ...
    const filteredData = data.filter((row) => {
        ...
        return (
            (row.nama_lengkap &&
                row.nama_lengkap.toLowerCase().includes(search)) ||
            (row.nip_nik_nipppk &&
                row.nip_nik_nipppk.toLowerCase().includes(search)) ||
    ...
    if (error)
        return (
            <Typography
                variant="h6"
                sx={{ mt: 10, textAlign: "center", color: "red" }}
            >
                {error}
            </Typography>
        );
    ...
    return (
        <>
            ...
            >
                <Table
                    aria-label="Tabel Pengelola GKT"
                ...
                >
                    <TableHead sx={{ backgroundColor: "#0C20B5" }}>
                        <TableRow>
                            ...
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "nip_nik_nipppk"}
                                    direction={order}
                                    onClick={() => handleSort("nip_nik_nipppk")}
                                    label="NIP / NIK / NIPPPK"
                                />
                            </TableCell>
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "jabatan"}
                                    direction={order}
            ...
            >
                <Alert
                    severity={
                        snackbarMessage.includes("Berhasil")
                            ? "success"
                            : "error"
                    }
                    onClose={handleCloseSnackbar}
