...
const DetailPenjadwalanGenap = () => {
  const { penjadwalangenapId } = useParams();
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/penjadwalangenap/detailgenap/${penjadwalangenapId}`);
  ...
  return (
    <div className="dashboard-container-jurusan">
      ...
      <div style={{ margin: "20px" }}>
        <h2>Detail Penjadwalan Semester Genap</h2>
        {detail ? (
          <Card>
            <Card.Body>
...
export default DetailPenjadwalanGenap;
