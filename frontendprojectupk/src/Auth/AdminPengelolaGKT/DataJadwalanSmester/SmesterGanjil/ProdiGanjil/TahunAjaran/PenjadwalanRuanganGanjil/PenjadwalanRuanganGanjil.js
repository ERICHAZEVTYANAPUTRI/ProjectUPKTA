...
const PenjadwalanRuangGanjil = () => {
  const { kodejurusanganjil, prodiganjilId, tahunAjaranganjilId, gedungId } = useParams();
  ...
  const handleSimpanRuangan = async (ruanganName) => {
    try {
      await axios.put(`http://localhost:8000/api/penjadwalanganjil/${selectedItemId}`, {
        namaruangan: ruanganName,
      });

      setPenjadwalanganjil((prev) => prev.map((item) => (item.id === selectedItemId ? { ...item, namaruangan: ruanganName } : item)));
  ...
  const handleKirimJadwal = async () => {
    try {
      const response = await axios.post(`http://localhost:8000/api/kirim-jadwal/${tahunAjaranganjilId}`);
      alert("Jadwal berhasil dikirim ke mahasiswa.");
      // Optional: Refresh data
      setPenjadwalanganjil((prev) => prev.map((item) => (item.statusterkirim === "belumterkirim" ? { ...item, statusterkirim: "terkirim" } : item)));
    } catch (error) {
  ...
  const handleChangeRuangan = (id, namaRuanganBaru) => {
    setPenjadwalanganjil((prev) => prev.map((item) => (item.id === id ? { ...item, namaruangan: namaRuanganBaru } : item)));
  };
  ...
  return (
    <div className="dashboard-container-jurusan">
      ...
      <h1 className="Dashboard-title" style={{ marginLeft: "10px" }}>
        Penjadwalan Ruang Kuliah Semester Ganjil
      </h1>
...
export default PenjadwalanRuangGanjil;
