...
import { Box, Button, IconButton, MenuItem, Modal, Select as MuiSelect, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Tooltip, Typography } from "@mui/material";
...
const HalamanAdminPengelolaPengembalianTidakSesuaiJadwal = () => {
  ...
  useEffect(() => {
    const fetchPengajuan = async () => {
      if (!user) return; // Jangan fetch sebelum user siap
  ...
  // Filter pengajuan diterima dengan kondisi tertentu
  const diterimaPengajuan = pengajuan.filter(
    (item) => (item.status === "diterima" && item.statuspeminjaman === "dipinjam") || (item.status === "prosespengembalian" && item.statuspeminjaman === "prosespengembalian" && item.statusuploadvidio && item.statusuploadvidio.trim() !== "")
  );
  ...
  const displayedItems = sortedPengajuan.slice(currentPage * rowsPerPage, (currentPage + 1) * rowsPerPage);
  ...
  const handleTolakVideo = async (pengajuanId) => {
    try {
      ...
      swal("Video Ditolak", "Mahasiswa Harus Upload Ulang Vidio Yang Sesuai Dengan ketentuan.", "info");

      setPengajuan((prev) => prev.map((p) => (p.id === pengajuanId ? { ...p, statusuploadvidio: "pending", statuspeminjaman: "dipinjam", status: "diterima" } : p)));
  ...
  const handleTerimaVideo = async (pengajuanId) => {
    try {
      ...
      swal("Berhasil!", "Data peminjaman disimpan sebagai selesai dan status jadwal diperbarui.", "success");

      setPengajuan((prev) => prev.map((p) => (p.id === pengajuanId ? { ...p, statusuploadvidio: "pending", statuspeminjaman: "dipinjam" } : p)));
      setShowVideoModal(false);
    } catch (error) {
      console.error(error);
      swal("Gagal!", "Terjadi kesalahan saat menolak video.", "error");
      ...
      <Modal open={showVideoModal} onClose={() => setShowVideoModal(false)} aria-labelledby="modal-video-title" closeAfterTransition>
        <Box sx={styleModal}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography id="modal-video-title" variant="h6" component="h2">
              Video Pengembalian
            </Typography>
          ...
          {videoUrl ? (
            ...
            <Typography sx={{ mb: 3, color: "red", fontWeight: 500 }}>Link Google Drive tidak valid atau tidak bisa ditampilkan. Namun Anda masih bisa menolak video ini.</Typography>
          )}

          <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 2 }}>
            <Button variant="contained" color="error" onClick={() => handleTolakVideo(selectedPengajuanId)}>
              Tolak
            </Button>
            ...
            <Button
              ...
              color="success"
              onClick={() => {
                if (selectedPengajuanId) handleTerimaVideo(selectedPengajuanId);
              }}
            >
              Terima
            </Button>
