...
const PenjadwalanRuanganAdminPengelola = () => {
    ...
    useEffect(() => {
        const fetchMasterData = async () => {
            try {
                ...
                const matkulFiltered = matkulRes.data.filter(
                    (m) => m.tahun_ajaran && m.tahun_ajaran.is_aktif === 1
                );
        ...
        const fetchData = async () => {
            try {
                const res = await axios.get(
                    "http://localhost:8000/api/penjadwalanruanganadminpengelola"
                );
                setData(res.data);
            } catch (error) {
    ...
    const handleGedungClick = async (gedungId) => {
        ...
        try {
            ...
            const responseRuangan = await axios.get(
            ...
            const responseTerpakai = await axios.post(
                `http://localhost:8000/api/penjadwalanruanganadminpengelola/ruangan-terpakai`,
                {
    ...
    const handleSimpanRuangan = async (ruangan_Id) => {
        try {
            ...
            await axios.put(
                `http://localhost:8000/api/penjadwalanruanganadminpengelola/updateruangan/${selectedItemId}`,
                {
            ...
            // Update state lokal
            const selectedRuangan = ruanganPilihan.find(
                (r) => r.id === ruangan_Id
            );
            setData((prev) =>
                prev.map((item) =>
            ...
            if (error.response && error.response.status === 422) {
                ...
                swal(
                    ...
                    error.response.data.message ||
                        "Ruangan sudah dijadwalkan pada hari dan jam tersebut.",
                    "warning"
                );
            } else {
                ...
                swal(
                    "Gagal!",
                    "Terjadi kesalahan saat menjadwalkan ruangan.",
                    "error"
                );
            }
        }
    ...
    const filteredData = data.filter((row) => {
        ...
        const matchesRuangan = selectedRuangan
            ? row.ruangan?.name === selectedRuangan
            : true;
        const matchesMatakuliah = selectedMatakuliah
            ? row.matakuliah?.namamatakuliah === selectedMatakuliah
            : true;
        const matchesDosen = selectedDosen
            ? row.dosen?.nama === selectedDosen
            : true;
        const matchesRuanganFilter = filterTanpaRuangan
            ? !row.ruangan_id
            : true;
        const matchesKelas = selectedKelas
            ? row.kelas?.nama === selectedKelas
            : true;
        const matchesJurusan = selectedJurusan
            ? row.mahasiswa?.jurusan?.namajurusan === selectedJurusan
            : true;
        const matchesProdi = selectedProdi
            ? row.prodi?.namaprodi === selectedProdi
            : true;

        return (
            matchesSearch &&
            matchesRuanganFilter &&
            matchesHari &&
            matchesRuangan &&
            matchesMatakuliah &&
            matchesDosen &&
                            ...
                            value={
                                selectedMatakuliah
                                    ? {
                                          ...
                                          label: selectedMatakuliah,
                                      }
                                    : null
                            }
                            onChange={(e) =>
                                setSelectedMatakuliah(e?.value || "")
                            }
                    ...
                    <FormControlLabel
                        control={
                            <Checkbox
                                ...
                                onChange={() =>
                                    setFilterTanpaRuangan((prev) => !prev)
                                }
                                sx={{
                                    "& .MuiSvgIcon-root": {
                                        fontSize: 18,
                    ...
                    <TableBody>
                        {displayedItems.map((item, index) => (
                            ...
                            >
                                ...
                                <TableCell sx={cellStyle}>
                                    <Tooltip
                                        title="Klik untuk memilih ruangan"
                                        arrow
                                    >
                                        <Box
                                            onClick={() =>
                                            ...
                                            sx={{
                                                ...
                                                alignItems: "center",
                                                gap: 0.5,
                                                color: item.ruangan?.name
                                                    ? "#0C20B5"
                                                    : "#1976d2",
                                                fontWeight: 600,
                                                cursor: "pointer",
                                                border: "1.5px solid",
                                                borderColor: item.ruangan?.name
                                                    ? "#0C20B5"
                                                    : "#1976d2",
                                                borderRadius: 1,
                                                padding: "4px 8px",
                                                transition: "all 0.3s ease",
                                                backgroundColor: item.ruangan
                                                    ?.name
                                                    ? "#f5f7ff"
                                                    : "#e3f2fd",
                                                "&:hover": {
                    ...
                    {!showRuanganOnly ? (
                        ...
                        <>
                            ...
                            >
                                ...
                                <Box sx={{ flex: "1" }}>
                                    <TextField
                                        ...
                                        onChange={(e) =>
                                            setSearchRuanganTerm(e.target.value)
                                        }
                                        fullWidth
                                        sx={{
                                            width: "100%",
                            ...
                            >
                                ...
                                >
                                    Ruangan Sudah Dijadwalkan
                                </Typography>
                            </Box>

                            {(() => {
                                const semuaRuanganCocok = ruanganPilihan.filter(
                                    (r) =>
                                        r.name
                                            .toLowerCase()
                                            .includes(
                                                searchRuanganTerm.toLowerCase()
                                            )
                                );
