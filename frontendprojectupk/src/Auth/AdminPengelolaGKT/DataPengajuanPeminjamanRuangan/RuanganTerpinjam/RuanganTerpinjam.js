...
const RuanganTerpinjam = () => {
  ...
  const diterimaPengajuan = pengajuan.filter((item) => item.status === "diterima" && item.statuspeminjaman === "dipinjam");

  const filteredData = diterimaPengajuan.filter((item) => {
    ...
    const namaruangan = item.ruangan?.name?.toLowerCase() || "";
    const jam = `${item.jammulai || ""} - ${item.jamselesai || ""}`.toLowerCase();

    const search = searchTerm.toLowerCase();
    return nama.includes(search) || kelas.includes(search) || matakuliah.includes(search) || dosen.includes(search) || namaruangan.includes(search) || jam.includes(search);
  });
...
export default RuanganTerpinjam;
