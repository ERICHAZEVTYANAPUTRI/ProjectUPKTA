...
const RuanganTerpinjamSesuaiJadwal = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/ruanganterpinjampengelolagkt");
        const filteredData = res.data.filter((item) => item.statuspeminjaman === "dipinjam");
  ...
  const handleSimpanRuangan = async (ruanganName) => {
    try {
      await axios.put(`http://localhost:8000/api/penjadwalanruanganganjiladminpengelola/updateruangan/${selectedItemId}`, {
        namaruangan: ruanganName,
      });

      setData((prev) => prev.map((item) => (item.id === selectedItemId ? { ...item, namaruangan: ruanganName } : item)));
