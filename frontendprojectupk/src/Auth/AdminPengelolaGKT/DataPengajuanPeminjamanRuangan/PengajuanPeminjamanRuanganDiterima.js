...
const PengajuanPeminjamanRuanganDiterima = () => {
        ...
        swal({
            ...
            if (willDelete) {
                try {
                    const token = localStorage.getItem("token");
                    await axios.patch(
                        `http://localhost:8000/api/pengajuan-peminjamans/${id}/batalkan`,
                        {},
                        { headers: { Authorization: `Bearer ${token}` } }
                    );
    ...
    const filteredPengajuan = pengajuan.filter((item) => {
        ...
        const namaruangan =
            item.namaruangan?.toLowerCase() ||
            item.ruangan?.name?.toLowerCase() ||
            "";
        const keperluan = item.keperluan?.toLowerCase() || "";
        const hari = item.hari?.toLowerCase() || "";
        const jurusan =
            item.mahasiswa?.jurusan?.namajurusan?.toLowerCase() || "";
        ...
        const filterMatch =
            ...
            (!selectedRuangan ||
                namaruangan === selectedRuangan.toLowerCase()) &&
            (!selectedMatakuliah ||
                matakuliah === selectedMatakuliah.toLowerCase()) &&
        ...
        return (
            ...
            item.statusuploadvidio === "pending" &&
            searchMatch &&
            filterMatch
        );
    });
    ...
    return (
        <>
                ...
                >
                    ...
                    >
                        <Select
                            options={[
                                {
                                    value: "/PengajuanPeminjamanRuangan",
                                    label: "Pengajuan Peminjaman Ruangan",
                                },
                                {
                                    value: "/pengajuanpeminjamanditerima",
                                    label: "Pengajuan Peminjaman Diterima",
                                },
                            ]}
                            value={
                                location.pathname ===
            ...
            >
                <Table
                    aria-label="Tabel Pengajuan Peminjaman"
                ...
                >
                    <TableHead sx={{ backgroundColor: "#0C20B5" }}>
                        <TableRow>
                            ...
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "matakuliah_id"}
                                    direction={order}
                                    onClick={() => handleSort("matakuliah_id")}
                                    label="Matakuliah"
                                />
                            </TableCell>
                            <TableCell sx={headStyle}>
                            ...
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "Batal"}
                                    direction={order}
                                    onClick={() => handleSort("Batal")}
                                    label="Batal"
                                />
                            </TableCell>
                        </TableRow>
