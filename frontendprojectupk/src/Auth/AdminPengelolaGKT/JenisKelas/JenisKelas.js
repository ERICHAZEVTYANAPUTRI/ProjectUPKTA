...
const Jeniskelas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/jeniskelas");
  ...
  const handleDelete = async (id) => {
    swal({
      ...
      text: "Setelah Dihapus Data Jenis Kelas Tidak dapat Dikembalikan Lagi!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
      ...
      if (willDelete) {
        try {
          await axios.delete(`http://localhost:8000/api/jeniskelas/${id}`);
          swal("Berhasil!", "Jenis Kelas Telah Dihapus.", "success");
          ...
          swal("Error!", "Gagal Menghapus Jenis Kelas.", "error");
        }
      } else {
        swal("Jenis Kelas Masih Tersimpan!");
      }
    });
  };
  ...
  const handleSaveJenisKelas = async (data, id = null) => {
    try {
      if (id) {
        await axios.put(`http://localhost:8000/api/jeniskelas/${id}`, data);
        swal("Berhasil!", "Data jeniskelas berhasil diperbarui.", "success");
      } else {
        await axios.post(`http://localhost:8000/api/jeniskelas`, data);
        swal("Berhasil!", "Data jeniskelas berhasil ditambahkan.", "success");
      }
      ...
      console.error("Gagal menyimpan data jeniskelas:", error);
  ...
  return (
    <>
      <ModalJenisKelas open={modalOpen} handleClose={closeModal} onSave={handleSaveJenisKelas} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
...
const ModalJenisKelas = ({ open, handleClose, onSave, initialData }) => {
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      ...
      >
        ...
        >
          {initialData ? "Edit Jenis Kelas" : "Tambah Jenis kelas"}
        </Typography>
        ...
        >
          ...
          <Tooltip title="Simpan Data Jenis kelas" arrow>
            <Box
