...
const DetailPeminjamanSelesaiAdminPengelola = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/api/peminjamanselesaidetailadminpengelola/${id}`);
  ...
  const { mahasiswa, matakuliah, prodi, tahunajaran, hari, jammulai, jamselesai, namaruangan, kebutuhankelas, statusjadwal, statusterkirim } = data || {};
  ...
  return (
    <div className="dashboard-container">
      ...
      <div style={{ marginLeft: "300px", padding: "30px" }}>
        <h2>Detail Jadwal Semester Ganjil</h2>
        ...
        <div className="d-flex gap-4 align-items-start">
          ...
          <table className="table table-striped w-75">
            <tbody>
              ...
              <tr>
                <th>Tahun Ajaran</th>
                <td>{tahunajaran?.namatahunajaran || "-"}</td>
              </tr>
              <tr>
                <th>Hari</th>
                <td>{hari || "-"}</td>
...
export default DetailPeminjamanSelesaiAdminPengelola;
