...
const HalamanAdminPengelolaPeminjamanSelesaiSesuaiJadwal = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        // Contoh endpoint bisa kamu sesuaikan sesuai backend kamu
        const [prodiRes, kelasRes, matakuliahRes, dosenRes, jurusanRes] = await Promise.all([
          axios.get("http://localhost:8000/api/prodi", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:8000/api/kelasmahasiswa", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:8000/api/matakuliah", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:8000/api/dosen", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:8000/api/jurusan", { headers: { Authorization: `Bearer ${token}` } }), // <-- Tambahkan ini
        ]);

        setProdiOptions(prodiRes.data.map((item) => ({ value: item.id, label: item.namaprodi })));
        setKelasOptions(kelasRes.data.map((item) => ({ value: item.id, label: item.nama })));
        setMatakuliahOptions(matakuliahRes.data.map((item) => ({ value: item.id, label: item.namamatakuliah })));
        setDosenOptions(dosenRes.data.map((item) => ({ value: item.id, label: item.nama })));
        setJurusanOptions(jurusanRes.data.map((item) => ({ value: item.id, label: item.namajurusan })));
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUserAndJadwal = async () => {
      try {
        ...
        const jadwalRes = await axios.post("http://localhost:8000/api/peminjamanselesaiadminpengelolaalldata", {}, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const filteredJadwal = jadwalMahasiswa.filter((item) => {
    ...
    const matchesHari = selectedHari ? item.hari === selectedHari.value : true;
    const matchesProdi = selectedProdi ? item.prodi?.id === selectedProdi.value : true;
    const matchesKelas = selectedKelas ? item.kelas?.id === selectedKelas.value : true;
    const matchesMatakuliah = selectedMatakuliah ? item.matakuliah?.id === selectedMatakuliah.value : true;
    const matchesDosen = selectedDosen ? item.dosen?.id === selectedDosen.value : true;
    const matchesJurusan = selectedJurusan ? item.mahasiswa?.jurusan?.id === selectedJurusan.value : true;

    return matchesSearch && matchesHari && matchesProdi && matchesKelas && matchesMatakuliah && matchesDosen && matchesJurusan;
  });

  const pageCount = Math.ceil(filteredJadwal.length / rowsPerPage);
  const displayedItems = filteredJadwal.slice(currentPage * rowsPerPage, (currentPage + 1) * rowsPerPage);
  ...
  const exportExcelSemuaDosen = (selectedMonth, selectedYear) => {
    ...
    filteredJadwal.forEach((item) => {
      ...
      if (itemMonth === selectedMonth && itemYear === selectedYear) {
        ...
        dosenGrouped[dosenName].push({
          ...
          Status: item.statusdialihkan === "dialihkan" ? "Dialihkan" : item.statusdigunakan === "digunakan" ? "Digunakan" : item.statustidakdigunakan || "-",
        });
      }
    });
    ...
    Object.entries(dosenGrouped).forEach(([dosen, data]) => {
      ...
      const safeSheetName = dosen.length > 31 ? dosen.slice(0, 28) + "..." : dosen;
    ...
    saveAs(dataBlob, `Jadwal_Semua_Dosen_Bulan_${selectedMonth}_${selectedYear}.xlsx`);
  };
    ...
    <>
      ...
      <Box sx={{ mb: 2 }}>
        <Box
          sx={{
            display: "flex",
            justifyContent: "space-between", // ⬅️ Tombol ke kanan
            alignItems: "center",
            flexWrap: "wrap",
            gap: 2,
            mb: 2,
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
            ...
            <Button
              variant="outlined"
              color="success" // ganti warna ke success (hijau)
              size="small"
              ...
              sx={{
                ...
                borderRadius: "4px",
                textTransform: "none",
                background: "linear-gradient(135deg, #28a745, #85d262)", // hijau gradient
                color: "#fff",
                boxShadow: "0 4px 12px rgba(40, 167, 69, 0.4)", // hijau shadow
                transition: "all 0.3s ease",
                "&:hover": {
                  background: "linear-gradient(135deg, #2ecc40, #a8e063)", // hijau hover
                  transform: "translateY(-1px)",
        ...
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 2 }}>
          <Select options={jurusanOptions} value={selectedJurusan} onChange={setSelectedJurusan} placeholder="Filter Jurusan" isClearable styles={{ container: (base) => ({ ...base, minWidth: 120, fontSize: "0.8rem" }) }} />
          <Select options={hariOptions} value={selectedHari} onChange={setSelectedHari} placeholder="Filter Hari" isClearable styles={{ container: (base) => ({ ...base, minWidth: 110, fontSize: "0.8rem" }) }} />
          <Select options={prodiOptions} value={selectedProdi} onChange={setSelectedProdi} placeholder="Filter Prodi" isClearable styles={{ container: (base) => ({ ...base, minWidth: 120, fontSize: "0.8rem" }) }} />
          <Select options={kelasOptions} value={selectedKelas} onChange={setSelectedKelas} placeholder="Filter Kelas" isClearable styles={{ container: (base) => ({ ...base, minWidth: 110, fontSize: "0.8rem" }) }} />
          <Select options={matakuliahOptions} value={selectedMatakuliah} onChange={setSelectedMatakuliah} placeholder="Filter Matakuliah" isClearable styles={{ container: (base) => ({ ...base, minWidth: 140, fontSize: "0.8rem" }) }} />
          <Select options={dosenOptions} value={selectedDosen} onChange={setSelectedDosen} placeholder="Filter Dosen" isClearable styles={{ container: (base) => ({ ...base, minWidth: 120, fontSize: "0.8rem" }) }} />
        </Box>
        <Divider sx={{ mb: 2 }} />
        ...
        <Table aria-label="Tabel Pengajuan Ruangan" sx={{ width: "100%", minWidth: 600, tableLayout: "auto" }}>
          <TableHead sx={{ backgroundColor: "#0C20B5" }}>
            <TableRow>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "mahasiswa_id"} direction={order} onClick={() => handleSort("mahasiswa_id")} label="Penggungjawab" />
              </TableCell>
              <TableCell sx={headStyle}>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "jurusan_id"} direction={order} onClick={() => handleSort("jurusan_id")} label="Jurusan" />
              </TableCell>
              <TableCell sx={headStyle}>
              ...
              <TableCell sx={headStyle}>
                <CustomSortLabel active={orderBy === "status"} direction={order} onClick={() => handleSort("status")} label="Status" />
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {displayedItems.map((item, index) => (
              <TableRow key={item.id} hover sx={{ backgroundColor: index % 2 === 0 ? "#f5f5f5" : "#ffffff", borderBottom: "1px solid #ccc" }}>
                ...
                <TableCell sx={cellStyle}>
                  {item.statusuploadvidio && item.statusuploadvidio.includes("/d/") ? (
                    ...
                    <Tooltip title="Video tidak tersedia" arrow>
                      <Box
                        sx={{
                          display: "inline-flex",
                          alignItems: "center",
                  ...
                  sx={{
                    ...
                    backgroundColor:
                      item.statusdialihkan === "dialihkan"
                        ...
                        ? "#e0f7fa" // biru muda
                        : "#f5f5f5", // abu default
                    color: "#000", // opsional: pastikan teks tetap terlihat
                    fontWeight: "bold", // agar teks lebih tegas
                  }}
                >
                  {item.statusdialihkan === "dialihkan" ? "Kelas Dialihkan" : item.statusdigunakan === "digunakan" ? "Kelas Masuk" : item.statustidakdigunakan === "kosong" ? "Kelas Kosong" : "-"}
                </TableCell>
              </TableRow>
            ))}
      ...
      >
        ...
        >
          <Box display="flex" flexDirection="column" gap={3}>
            <TextField select label="Tahun" value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))} fullWidth size="small" variant="outlined">
            ...
            <TextField select label="Bulan" value={selectedMonth} onChange={(e) => setSelectedMonth(parseInt(e.target.value))} fullWidth size="small" variant="outlined">
        ...
        <DialogActions sx={{ px: 4, pb: 3, justifyContent: "flex-end" }}>
          <Button onClick={() => setShowExportModal(false)} variant="outlined" color="inherit" size="small" sx={{ textTransform: "none", borderRadius: 2 }}>
            Batal
          </Button>
          <Button
            onClick={() => {
