...
const TabelMahasiswa = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const filteredData = userRes.data.filter(
    ...
    const filteredData = data.filter((row) => {
        ...
        const jurusan =
            row.jurusan && row.jurusan.namajurusan
                ? row.jurusan.namajurusan.toLowerCase()
                : "";
        const username = (row.username || "").toLowerCase();
        const namaLengkap = (row.nama_lengkap || "").toLowerCase();
        const prodi =
            row.prodi && row.prodi.namaprodi
                ? row.prodi.namaprodi.toLowerCase()
                : "";
        const kelas =
            row.kelas && row.kelas.nama ? row.kelas.nama.toLowerCase() : "";
        ...
        return (
            ...
            nim.includes(search) ||
            jurusan.includes(search) ||
            notlp.includes(search)
        );
    });
                ...
                >
                    <TableHead sx={{ backgroundColor: "#0C20B5" }}>
                        <TableRow>
                            ...
                            <TableCell sx={headStyle}>
                                <CustomSortLabel
                                    active={orderBy === "notlp"}
                                    direction={order}
                                    onClick={() => handleSort("notlp")}
                                    label="No.Telpon"
                                />
                            </TableCell>
                            <TableCell sx={headStyle}>
