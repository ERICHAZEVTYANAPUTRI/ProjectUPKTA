...
const Ruangan = () => {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                ...
                setJenisKelasOptions(
                    jenis.data.map((item) => ({
                        value: String(item.id),
                        label: item.nama,
                    }))
                );
                setModelKelasOptions(
                    model.data.map((item) => ({
                        value: String(item.id),
                        label: item.nama,
                    }))
                );
                setSaranaKelasOptions(
    ...
    const handleMultiSelectChange = (selected, name, index) => {
        ...
        updatedRooms[index][name] = selected;
    ...
    const handleSubmit = async (e) => {
        ...
        rooms.forEach((room, index) => {
            ...
            const jeniskelasValues =
                room.jeniskelas?.map((item) => item.value) || [];
            const modelkelasValues =
                room.modelkelas?.map((item) => item.value) || [];
            const saranakelasValues =
                room.saranakelas?.map((item) => item.value) || [];

            jeniskelasValues.forEach((val, i) =>
                formData.append(`jeniskelas[${index}][${i}]`, val)
            );
            modelkelasValues.forEach((val, i) =>
                formData.append(`modelkelas[${index}][${i}]`, val)
            );
            saranakelasValues.forEach((val, i) =>
                formData.append(`saranakelas[${index}][${i}]`, val)
            );
        ...
        try {
            const response = await axios.post(
                "http://localhost:8000/api/ruangan",
                formData,
                {
            ...
            const parsedRooms = newRooms.map((room) => ({
                ...parseRoomData(room),
                statusruangan: room.statusruangan || "kosong",
            }));
        ...
        if (selectedRoom) {
            ...
            const mapToOptions = (ids, options) => {
                ...
                return options.filter((option) =>
                    ids.includes(String(option.value))
                );
            };

            const jeniskelasIDs = Array.isArray(selectedRoom.jeniskelas)
                ? selectedRoom.jeniskelas
                : JSON.parse(selectedRoom.jeniskelas || "[]");
            const modelkelasIDs = Array.isArray(selectedRoom.modelkelas)
                ? selectedRoom.modelkelas
                : JSON.parse(selectedRoom.modelkelas || "[]");
            const saranakelasIDs = Array.isArray(selectedRoom.saranakelas)
                ? selectedRoom.saranakelas
                : JSON.parse(selectedRoom.saranakelas || "[]");

            setFormData({
                name: selectedRoom.name,
                lantai: selectedRoom.lantai,
                kapasitas: selectedRoom.kapasitas,
                jeniskelas: jenisKelasOptions.filter((opt) =>
                    jeniskelasIDs.includes(opt.value)
                ),
                modelkelas: modelKelasOptions.filter((opt) =>
                    modelkelasIDs.includes(opt.value)
                ),
                saranakelas: saranaKelasOptions.filter((opt) =>
                    saranakelasIDs.includes(opt.value)
                ),
            });
        }
    ...
    const handleUpdateRoom = async () => {
        try {
            const jeniskelasValues = formData.jeniskelas
                ? formData.jeniskelas.map((item) => String(item.value))
                : [];
            const modelkelasValues = formData.modelkelas
                ? formData.modelkelas.map((item) => String(item.value))
                : [];
            const saranakelasValues = formData.saranakelas
                ? formData.saranakelas.map((item) => String(item.value))
                : [];
            ...
            const response = await axios.put(
                `http://localhost:8000/api/perbaruiruangan/${selectedRoom.id}`,
                payload
            );
            ...
            case "updateStatus":
                swal({
                    ...
                    if (willUpdate) {
                        axios
                            .put(
                                `http://localhost:8000/api/ruangan/${selectedRoom.id}`,
                                { statusruangan: "diperbaiki" }
                            )
                            .then(() => {
                                setRuangan(
                                    ruangan.map((room) =>
        ...
        swal({
            ...
            if (willDelete) {
                try {
                    ...
                    setRuangan(
                        ruangan.filter((room) => room.id !== selectedRoom.id)
                    );
                    swal({
                        title: "Sukses!",
        ...
        <>
            ...
            >
                <MenuList disablePadding>
                    ...
                    {selectedRoom &&
                        selectedRoom.statusruangan === "diperbaiki" && (
                            <>
                                <MenuItem
                                    sx={menuItemStyle}
                                    onClick={() => handleAction("detail")}
                                ...
                                <MenuItem
                                    sx={menuItemStyle}
                                    onClick={handleUpdateStatusToKosong}
                                >
                    ...
                    {selectedRoom &&
                        selectedRoom.statusruangan === "dipinjam" && (
                            <MenuItem
                                sx={menuItemStyle}
                                onClick={() => handleAction("detail")}
                            >
