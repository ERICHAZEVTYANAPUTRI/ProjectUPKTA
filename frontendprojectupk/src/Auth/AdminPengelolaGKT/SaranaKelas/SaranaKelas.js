...
const SaranaKelas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/saranakelas");
  ...
  const handleDelete = async (id) => {
    swal({
      ...
      text: "Setelah Dihapus Data Sarana Kelas Tidak dapat Dikembalikan Lagi!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
      ...
      if (willDelete) {
        try {
          await axios.delete(`http://localhost:8000/api/saranakelas/${id}`);
          swal("Berhasil!", "Sarana Kelas Telah Dihapus.", "success");
          ...
          swal("Error!", "Gagal Menghapus Sarana Kelas.", "error");
        }
      } else {
        swal("Sarana Kelas Masih Tersimpan!");
      }
    });
  };
  ...
  const handleSaveSaranaKelas = async (data, id = null) => {
    try {
      if (id) {
        await axios.put(`http://localhost:8000/api/saranakelas/${id}`, data);
        swal("Berhasil!", "Data saranakelas berhasil diperbarui.", "success");
      } else {
        await axios.post(`http://localhost:8000/api/saranakelas`, data);
        swal("Berhasil!", "Data saranakelas berhasil ditambahkan.", "success");
      }
      ...
      console.error("Gagal menyimpan data saranakelas:", error);
  ...
  return (
    <>
      <ModalSaranakelas open={modalOpen} handleClose={closeModal} onSave={handleSaveSaranaKelas} initialData={editData} />
      <Navbar />
      <Box sx={{ mb: 2 }}>
...
const ModalSaranakelas = ({ open, handleClose, onSave, initialData }) => {
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      ...
      >
        ...
        >
          {initialData ? "Edit Sarana Kelas" : "Tambah Sarana kelas"}
        </Typography>
        ...
        >
          ...
          <Tooltip title="Simpan Data Sarana kelas" arrow>
            <Box
