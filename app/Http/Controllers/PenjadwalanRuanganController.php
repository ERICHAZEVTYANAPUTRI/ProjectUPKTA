...
class PenjadwalanRuanganController extends Controller
{
    public function pinjamJadwal(Request $request, $id)
    {
        ...
        if ($jadwal->statuspeminjaman === 'dipinjam') {
            return response()->json(['message' => 'Jadwal sudah dipinjam.'], 400);
        }

        if ($jadwal->statuspeminjaman === 'prosespengembalian') {
            return response()->json(['message' => 'Jadwal sedang dalam proses pengembalian.'], 400);
        }
        ...
        try {
            $jamSelesai = Carbon::createFromFormat('H:i:s', $jadwal->jamselesai);
        } catch (\Exception $e) {
            $jamSelesai = Carbon::createFromFormat('H:i', $jadwal->jamselesai);
        }

        $jamMulaiFormatted = $jamMulai->format('H:i');
        $jamSelesaiFormatted = $jamSelesai->format('H:i');
        $hariFormatted = ucfirst($jadwal->hari);
        $now = now();

        if ($hariSekarang !== $hariJadwal) {
            return response()->json([
                'message' => "Jadwal ini hanya dapat dipinjam pada hari $hariFormatted pukul $jamMulaiFormatted - $jamSelesaiFormatted."
            ], 400);
        }

        if ($now->lt($jamMulai) || $now->gt($jamSelesai)) {
            return response()->json([
                'message' => "Peminjaman hanya dapat dilakukan antara pukul $jamMulaiFormatted hingga $jamSelesaiFormatted.",
            ], 400);
        }
    ...
    public function getJadwalByRuanganAndHari(Request $request)
    {
        ...
        $jadwal = PenjadwalanRuangan::with('kelas')
            ->where('hari', $hari)
            ->where('ruangan_id', $ruanganId)
            ->whereHas('tahunajaran', function ($query) {
                $query->where('is_aktif', 1);
            })
            ->select('id', 'jammulai', 'jamselesai', 'kelas_id', 'hari')
            ->get();
    ...
    public function jadwalHarian($id, Request $request)
    {
        ...
        $jadwal = PenjadwalanRuangan::with(['kelas', 'tahunAjaran'])
            ->where('ruangan_id', $id)
            ->where('hari', $hari)
            ->where('statusterkirim', 'terkirim')
            ->whereHas('tahunAjaran', function ($query) {
