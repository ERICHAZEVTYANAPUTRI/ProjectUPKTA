...
try {
    ...
    DB::transaction(function () use ($request, $user) {
        ...
        // Hanya update jurusan jika role adalah admin_jurusan
        if ($user->role === 'admin_jurusan') {
            $jurusan = Jurusan::where('kodejurusan', $kodeLama)->first();

            if (!$jurusan) {
                // Kunci: throw agar transaksi gagal
...
public function show($id)
{
    try {
        $user = User::findOrFail($id);
        return response()->json($user, 200); // Return user data as JSON
    } catch (\Exception $e) {
...
public function destroy($id)
{
    try {
        ...
        // Hapus jurusan terkait jika ada
