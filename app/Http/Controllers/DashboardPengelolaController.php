...
class DashboardPengelolaController extends Controller
{
    ...
    {
        $pengajuanPeminjaman = DB::table('pengajuan_peminjaman_ruangans')
            ...
            ->where('pengajuan_peminjaman_ruangans.statuspeminjaman', 'prosespengembalian')
            ->get();
        ...
        $penjadwalan = DB::table('penjadwalan_ruangans')
            ...
            ->where('penjadwalan_ruangans.statuspeminjaman', 'prosespengembalian')
            ->get();

        $pengajuanKegiatan = DB::table('pengajuan_kegiatan')
            ->join('users', 'pengajuan_kegiatan.mahasiswa_id', '=', 'users.id')
            ->select(
                'pengajuan_kegiatan.id',
                'users.nama_lengkap',
                'users.nim',
                'users.foto',
                'pengajuan_kegiatan.keperluan',
                'pengajuan_kegiatan.jammulai',
                'pengajuan_kegiatan.jamselesai'
            )
            ->where('pengajuan_kegiatan.status', 'prosespengembalian')
            ->get();

        $result = $pengajuanPeminjaman->merge($penjadwalan)->merge($pengajuanKegiatan);
